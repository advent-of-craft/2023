...
describe('population', () => {
    ...
    test('we should be able to represent people with their pets', () => {
        expect(formatPopulation(population))
            .toBe("Peter Griffin who owns : Tabby " + '\n' +
...
const formatPopulation = (population: Person[]): string => population
    .map(formatPerson)
    .join('\n');

const formatPerson = (person: Person): string => `${person.firstName} ${person.lastName}` + formatPets(person.pets)
const formatPets = (pets: Pet[]): string => pets.length > 0
    ? ` who owns : ${pets.map(p => p.name).join(" ")} `
    : ""
